...
| `full_trend` | 含24h价格趋势图 | 先点击趋势图进入价格详情，再滚动2次并合并；详情数据优先、趋势图兜底 |
...
- **采集字段**：
  ...
  - 停车费、占位费、设施标签、收藏数
  - 高德 API 逆地理编码获取经纬度
- **分时电价采集**：价格详情页数据优先；点击失败、解析为空或详情缺少时段时，使用站点页内嵌趋势数据兜底
- **页面强校验**：点击价格入口后必须识别为 `PRICE_DETAIL`，返回后必须确认仍是原站点详情，避免误点停车费等无关“查看详情”
