...
电脑重启后，之前手工启动的 API 进程会被系统结束。为避免手机继续运行时出现“同步失败”，首次配置完成后可执行一次：

```powershell
./install_backend_autostart.ps1
```

它会在当前 Windows 用户的“启动”目录创建后台启动项。以后每次电脑重启并登录 Windows，都会自动检查并启动 8800 服务；如果服务已经运行，不会重复启动。手机端的结果会继续保存在本地队列，服务恢复后自动补传，不需要重新采集已完成站点。
